...
def plot_error_dynamic(filter_results_npz):
    '''
    plot position and rotation errors
    '''
    ...
    for i, ti in enumerate(t):
        ...
        sp1 = pp.subplot(2,1,1)
        sp1.set_title('position error', fontsize=title_font_size)
        pp.plot(t, xyz_err_meas,
                label='neural network (mean = %.1f)' % xyz_err_mean_meas,
                color=net_clr)
        pp.plot(t, xyz_err,
                label='filter \hspace{52pt} (mean = %.1f)' % xyz_err_mean,
                color=filter_clr, linestyle=':')
        pp.plot(t[i], xyz_err_meas[i], color=net_clr, marker='o') # plot ball 1
        pp.plot(t[i], xyz_err[i], color=filter_clr, marker='o') # plot ball 2
        pp.xlabel('time [s]', fontsize=xlabel_font_size)
        pp.ylabel('$\| \hat{\mathbf{p}} - \mathbf{p} \|$ [cm]',
                  fontsize=xlabel_font_size)
        pp.xticks(fontsize=tick_font_size)
        ...
        pp.plot(t, R_err_meas, 'k',
                ...
                color=net_clr)
        pp.plot(t, R_err,
                label='filter \\hspace{52pt} (mean = %.1f)' % R_err_mean,
                color=filter_clr, linestyle=':')
        pp.plot(t[i], R_err_meas[i], color=net_clr, marker='o') # plot ball 1
        pp.plot(t[i], R_err[i], color=filter_clr, marker='o') # plot ball 2
        pp.xlabel('time [s]', fontsize=xlabel_font_size)
        pp.ylabel('$\\text{dist}(\\widehat{\\mathbf{R}}, \\mathbf{R})$ [\\textdegree]',
                  fontsize=xlabel_font_size)
        pp.xticks(fontsize=tick_font_size)
        ...
        # save
        file_name_i = 'plot_' + str(i) + '.png'
        file_path_i = os.path.join(dirs.simulation_dynamic_err_dir, file_name_i)
        pp.savefig(file_path_i, dpi=300)
        pp.close()
