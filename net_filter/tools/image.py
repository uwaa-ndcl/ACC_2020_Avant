...
def point_to_pix(w, h, d, aov_w, aov_h, pix_w, pix_h):
    '''
    convert the x,y,z location of a point to pixel space
    w: width, h: height, d: depth of a 3D point
    '''

    # maxes and ratios for given depth
    w_max = d*np.tan(aov_w/2) # aov is full angle, so we halve it
    h_max = d*np.tan(aov_h/2)
    ratio_w = w/w_max # between -1 and 1
    ratio_h = h/h_max

    # normalize
    w_normal = .5*ratio_w + .5 # scale and shift to put between 0 and 1
    h_normal = .5*ratio_h + .5

    # to pixels
    w_pix = w_normal * pix_w
    h_pix = h_normal * pix_h
    pix = np.block([[w_pix], [h_pix]])

    return pix


def draw_box(input_file, output_file, box):
    '''
    draw a box on an image
    box: u and v coordinates of box in pixels
    '''

    clr = (255, 0, 0)
    im = load_im_np(input_file)
    # below: note image is of shape (height, width) NOT (width, height)
    pix_h, pix_w = im.shape[:2]
    # our y is down-to-up, but pixel space is up-to-down,
    # so flip the y-axis
    box[1,:] = pix_h - box[1,:]
    ...
    # edges
    cv2.line(im, pt_0, pt_1, clr, thickness=1)
    cv2.line(im, pt_0, pt_2, clr, thickness=1)
    cv2.line(im, pt_0, pt_4, clr, thickness=1)
    cv2.line(im, pt_1, pt_3, clr, thickness=1)
    cv2.line(im, pt_1, pt_5, clr, thickness=1)
    cv2.line(im, pt_2, pt_3, clr, thickness=1)
    cv2.line(im, pt_2, pt_6, clr, thickness=1)
    cv2.line(im, pt_3, pt_7, clr, thickness=1)
    cv2.line(im, pt_4, pt_5, clr, thickness=1)
    cv2.line(im, pt_4, pt_6, clr, thickness=1)
    cv2.line(im, pt_5, pt_7, clr, thickness=1)
    cv2.line(im, pt_6, pt_7, clr, thickness=1)

    write_im_np(output_file, im)
