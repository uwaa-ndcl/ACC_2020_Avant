...
from net_filter.dope.cuboid_pnp_solver import *
...
class ObjectDetector(object):
    ...
    def detect_object_in_image(net_model, pnp_solver, in_img, config):
        ...
        # TDA: save vertex2 and aff
        vertex2_np = vertex2.cpu().data.numpy()
        aff_np = aff.cpu().data.numpy()

        # Find objects from network output
        detected_objects, maps = ObjectDetector.find_object_poses(vertex2, aff, pnp_solver, config) #TDA

        if not detected_objects:
            print('no object found in detect_object_in_image()')

        return detected_objects, vertex2_np, aff_np, maps #TDA
    ...
    def find_object_poses(vertex2, aff, pnp_solver, config):
        ...
        objects, all_peaks, maps = ObjectDetector.find_objects(vertex2, aff, config) #TDA
        detected_objects = []
        ...
        for obj in objects:
            ...
            # TDA: make sure there are enough points to do PnP
            if 9 - points.count(None) >= 6:  
        ...
        return detected_objects, maps #TDA
    ...
    def find_objects(vertex2, aff, config, numvertex=8):
        ...
        # TDA: make maps variable
        vertex2_shape = vertex2.cpu().data.numpy().shape
        maps = np.full(vertex2_shape, np.nan)
        ...
        return objects, all_peaks, maps #TDA
